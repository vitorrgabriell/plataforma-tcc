...
from app.schemas import (
    ...
    ResgateFidelidadeCreate,
    ResgateFidelidadeResponse,
)
...
def criar_programa_fidelidade(
    programa: ProgramaFidelidadeCreate, db: Session = Depends(get_db)
):
...
def meus_pontos(user=Depends(get_current_user)):
    if user["tipo_usuario"] != "cliente":
        raise HTTPException(
            status_code=403, detail="Apenas clientes podem acessar seus pontos."
        )
...
def listar_pontos_por_estabelecimento(
    ...
    pontos = (
        ...
        .options(joinedload(PontosFidelidadeCliente.estabelecimento))
        .filter(PontosFidelidadeCliente.cliente_id == user["id"])
        .all()
    )
        ...
        resultado.append(
            {
                ...
                "estabelecimento_nome": (
                    ponto.estabelecimento.nome
                    if ponto.estabelecimento
                    else "Desconhecido"
                ),
...
def acumular_ponto(usuario_id: int, db: Session = Depends(get_db)):
    ...
    if usuario.tipo_usuario != "cliente":
        raise HTTPException(
            status_code=400, detail="Somente clientes podem acumular pontos"
        )
...
def resgatar_premio(resgate: ResgateFidelidadeCreate, db: Session = Depends(get_db)):
    ...
    if cliente.tipo_usuario != "cliente":
        raise HTTPException(
            status_code=400, detail="Somente clientes podem resgatar prêmios"
        )
    ...
    if not programa:
        raise HTTPException(
            status_code=404, detail="Programa de fidelidade não encontrado"
        )

    pontos_cliente = (
        db.query(PontosFidelidadeCliente)
        .filter_by(
            cliente_id=cliente.id, estabelecimento_id=programa.estabelecimento_id
        )
        .first()
    )
    ...
    if (
        not pontos_cliente
        or pontos_cliente.pontos_acumulados < programa.pontos_necessarios
    ):
        raise HTTPException(
            status_code=400, detail="Pontos insuficientes para resgatar o prêmio"
        )

    pontos_cliente.pontos_acumulados -= programa.pontos_necessarios

    novo_resgate = ResgateFidelidade(
        cliente_id=cliente.id, programa_fidelidade_id=programa.id
    )
    db.add(novo_resgate)
...
def atualizar_programa_fidelidade(
    programa_id: int, dados: ProgramaFidelidadeUpdate, db: Session = Depends(get_db)
):
...
def obter_ultimo_servico_cliente(user=Depends(get_current_user)):
    if user["tipo_usuario"] != "cliente":
        raise HTTPException(
            status_code=403, detail="Apenas clientes podem acessar o último serviço."
        )
