...
def listar_funcionarios(
    ...
    if user["tipo_usuario"] not in ["admin", "cliente"]:
        raise HTTPException(status_code=403, detail="Apenas administradores ou clientes podem acessar.")

    return db.query(Funcionario).filter(Funcionario.estabelecimento_id == estabelecimento_id).all()
