...
# 📌 GET: Listar funcionários por estabelecimento (apenas para usuários autenticados com vínculo)
@router.get("/", response_model=list[FuncionarioResponse])
def listar_funcionarios(
    ...
    if user["tipo_usuario"] != "admin":
        raise HTTPException(status_code=403, detail="Apenas administradores podem listar funcionários.")
    
    funcionarios = db.query(Funcionario).filter(Funcionario.estabelecimento_id == estabelecimento_id).all()
    return funcionarios

# 📌 POST: Cadastrar funcionário (apenas usuários vinculados a um estabelecimento)
@router.post("/", response_model=dict)
def cadastrar_funcionario(
    funcionario: FuncionarioCreate,
    db: Session = Depends(get_db),
