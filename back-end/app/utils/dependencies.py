...
#         return {"email": user_email, "id": user_id, "tipo_usuario": user_role, "estabelecimento_id": estabelecimento_id}
...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
        print("Payload decodificado:", payload)  # ADICIONE ESSA LINHA PARA DEBUG
